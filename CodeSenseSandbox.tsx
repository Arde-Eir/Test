...
const CodeSenseSandbox = () => {
    ...
    const handleAnalyze = () => {
        ...
        const diagnostics = engine.analyze(ast);
        const explanations = ast.map(n => pedagogy.translate(n));

        // Algorithms 8 & 9: CFG Construction & Sugiyama Layout [cite: 730, 734]
        // Transforms linear code into the visual GPS map[cite: 240, 241].
        ...
        setLogs([...diagnostics, ...explanations]);
        if (diagnostics.length === 0) setTokens(t => t + pedagogy.calculateReward(ast));
    };
    ...
    return (
        <div className="pixel-container" style={{ background: '#1a1a1a', height: '100vh', padding: '20px', color: '#f1c40f' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '20px', height: 'calc(100% - 100px)' }}>
